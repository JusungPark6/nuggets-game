...
bool is_visible(grid_t* grid, const int pc, const int pr, const int c, const int r){
    // EXTRA CREDIT: "Set a range limit on vision, e.g., a diameter of five spots."
    double max_visibility_diameter = 5;
    // Calculate the distance between the player and the spot being checked
    double distance = sqrt(pow((c - pc), 2) + pow((r - pr), 2));
    // If the distance is greater than the maximum visibility range, it's not visible
    if (distance > max_visibility_diameter) { 
        return false;
    }
    ...
    for(int i = 0; i < width; i++){
        for(int j = 0; j < width + 1 ; j++){
            ...
            if(old_temp == '*' && merge_temp != '*' ){
                if(merge_temp != '@' && !isalpha(merge_temp)){
                    grid_change_char_at(old, i,j, '.');
                } else if (merge_temp=='@') {
                    grid_change_char_at(old, i,j, '@');
                }
                else if (isalpha(merge_temp)) {
